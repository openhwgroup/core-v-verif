...
# LIST_EXCLUDE_TEST_OPTIONS=(0 1 2 3 4 5)
...
# Build:corev-dv 
echo "regr: Running build: [cd ${WORKSPACE}/cv32e40p/sim/uvmt && make comp_corev-dv CV_CORE=cv32e40p CFG=$TEST_CFG SIMULATOR=${SIMULATOR} USE_ISS=$USE_ISS COV=$COV  ]"
pushd ${WORKSPACE}/cv32e40p/sim/uvmt > /dev/null
make comp_corev-dv CV_CORE=cv32e40p CFG=$TEST_CFG SIMULATOR=${SIMULATOR} USE_ISS=$USE_ISS COV=$COV  
popd > /dev/null
...
pushd ${WORKSPACE}/cv32e40p/sim/uvmt > /dev/null
make comp CV_CORE=cv32e40p CFG=$TEST_CFG SIMULATOR=${SIMULATOR} USE_ISS=$USE_ISS COV=$COV  
popd > /dev/null
...
if [[ ${LIST_EXCLUDE_TEST_OPTIONS[*]} =~ $n ]]; then
  ...
  # sanity test
  pushd ${WORKSPACE}/cv32e40p/sim/uvmt > /dev/null
  make gen_corev-dv test TEST=corev_sanity_fp${IS_ZFINX}_instr_test CV_CORE=cv32e40p CFG=$TEST_CFG COREV=1 RISCVDV_CFG= SIMULATOR=${SIMULATOR} COMP=0 USE_ISS=$USE_ISS COV=$COV SEED=$TEST_SEED GEN_START_INDEX=$m RUN_INDEX=$m   >& /dev/null;
  popd > /dev/null
