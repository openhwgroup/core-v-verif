...
make clean_all
python3 cva6.py --target cv32a6_imac_sv0 --iss=$DV_SIMULATORS --iss_yaml=cva6.yaml --c_tests ../tests/custom/coremark/core_main.c --gcc_opts '../tests/custom/coremark/core_list_join.c ../tests/custom/coremark/core_matrix.c ../tests/custom/coremark/core_portme.c ../tests/custom/coremark/core_state.c ../tests/custom/coremark/core_util.c -O3 -fno-tree-loop-distribute-patterns -nostdlib -nostartfiles -DFLAGS_STR="-O3_-fno-tree-loop-distribute-patterns" -DITERATIONS=10 -DPERFORMANCE_RUN -g -nostdlib -nostartfiles ../tests/custom/common/syscalls.c ../tests/custom/common/crt.S -I../tests/custom/env -I../tests/custom/common -lgcc'
make -C ../../core-v-cores/cva6 clean
make clean_all
