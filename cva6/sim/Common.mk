...
# Makefiles in the "core" and "uvm" directories.
#
###############################################################################
# Common functions
...
CVA6_REPO         ?= https://github.com/MikeOpenHWGroup/cva6
CVA6_BRANCH       ?= experimental
CVA6_HASH         ?= head
CVA6_TAG          ?= none
CVA6_WORK_DIR     ?= $(PROJ_ROOT_DIR)/core-v-cores/cva6
...
ifeq ($(CVA6_BRANCH), master)
  TMP = git clone --recursive $(CVA6_REPO) $(CVA6_WORK_DIR)
else
  TMP = git clone --recursive -b $(CVA6_BRANCH) --single-branch $(CVA6_REPO) $(CVA6_WORK_DIR)
endif
...
ifeq ($(CVA6_TAG), none)
  ifeq ($(CVA6_HASH), head)
    ...
    CLONE_CVA6_CMD = $(TMP); cd $(CVA6_WORK_DIR); git checkout $(CVA6_HASH)
  endif
else
  CLONE_CVA6_CMD = $(TMP); cd $(CVA6_WORK_DIR); git checkout tags/$(CVA6_TAG)
endif
...
###############################################################################
# Clone CVA6 RTL and DV dependencies
clone_cva6_rtl:
	$(CLONE_CVA6_CMD)
...
###############################################################################
# Rules for cleanup, misc

clean_bsp:
	make clean -C $(BSP)

clean_cva6_rtl:
	@echo "[COMMON] Remove CVA6 RTL working directory"
	rm -rf $(CVA6_WORK_DIR)

show_vars:
	@echo "PROJ_ROOT_DIR = $(PROJ_ROOT_DIR)"
	@echo "CVA6_WORK_DIR = $(CVA6_WORK_DIR)"
	@echo "BSP           = $(BSP)"
