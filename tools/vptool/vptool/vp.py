...
# Script location is defined in vp_config.py.
global SCRIPT_LOCATION, PROJECT_NAME
from vp_pack import *
import os, sys, pwd
...
class MyListWidget(ttk.LabelFrame):
	...
	"""
	def __init__(self,master=None,MasterClass=None,reflist=[],text="",expand="yes",side=None,padx=1,pady=2,bdelfunc=None,baddfunc=None,width=20,height=4,spinfunc=None,spin_value=None,spin_vip_value=None,bdel_default="normal", style=None):
		...
		self.wlist = tk.Listbox(self.lframe,exportselection=0,width=width,height=height,bg=BG_COLOR)
		self.sbar = ttk.Scrollbar(self.lframe)
		self.badd = ttk.Button(self.bframe, text="New", command=self.badd_action, state="disabled")
		self.bdel = ttk.Button(self.bframe, text="Del", command=self.bdel_action, state="disabled")
	...
	def pack(self,side,expand,padx,pady):
		...
		ttk.LabelFrame.pack(self, expand=expand, fill='both', side=side, padx=padx, pady=pady)
...
class MyMenuWidget(tk.Menu):
	def __init__(self,master=None,MasterClass=None,savfunc="",loadfunc="",closefunc="",renamefunc="",lockfunc="",duplicate_itemfunc="",pref_save_func="",pref_color_func="",exitfunc=""):
	...
	def menu_config(self,master):
		...
		self.viewmenu.add_command(label="--- Lock/Unlock Feature    ",command=self.lockfunc)
		self.viewmenu.add_separator()
		self.viewmenu.add_command(label="--- Rename Feature       ",command=self.renamefunc)
		self.viewmenu.add_command(label="--- Define Property Custom Numbering   ",command=self.set_prop_custom)
		self.viewmenu.add_checkbutton(label="--- Enable Feature/Sub-Feature Reordering   ",onvalue=1, offvalue=0, variable=self.enable_reordering, command=self.reordering)
		self.editmenu.add_command(label="--- Duplicate item       ",command=self.duplicate_itemfunc)
		self.editmenu.add_command(label="--- Copy and insert item ",command=self.insert_itemfunc)
		self.editormenu.add_checkbutton(label=" nedit  ",onvalue=1, variable=self.editor_choice, command=self.update_editor)	
		self.editormenu.add_checkbutton(label=" geany  ",onvalue=2, variable=self.editor_choice, command=self.update_editor)	
		self.editormenu.add_checkbutton(label=" gvim  ",onvalue=3, variable=self.editor_choice, command=self.update_editor)	
		self.editormenu.add_checkbutton(label=" gedit  ",onvalue=4, variable=self.editor_choice, command=self.update_editor)	
		self.add_cascade(label="File  ", menu=self.filemenu)
		self.add_cascade(label="Edit  ", menu=self.editmenu)
		self.add_cascade(label="Function  ", menu=self.viewmenu)
		#self.add_cascade(label="Export  ", menu=self.exportmenu)
		self.prefmenu.add_command(label="--- Color Settings  ",command=self.pref_color_func)
		self.prefmenu.add_cascade(label="--- Editor  ", menu=self.editormenu)
		self.prefmenu.add_separator()
		self.prefmenu.add_command(label="--- Save Preferences ",command=self.pref_save_func)
	...
	def set_prop_custom(self):
		global CUSTOM_NUM
		change_custom_name = tkinter.simpledialog.askstring('Section Numbering',prompt='Specify a section custom numbering if needed',parent=self.MasterClass.top,initialvalue=CUSTOM_NUM)
...
class MyTextWidget(ttk.LabelFrame):
	...
	def __init__(self,master=None, parent=None, text="",expand=True,side=None,padx=1,pady=2,width=20,bsavefunc=None,bcancelfunc=None,pfc_func=None,testtype_func=None,covmethod_func=None):
		global yaml_config
		ttk.LabelFrame.__init__(self, master,text=text)
		self.parent = parent

		enabledLabelFrame_style = ttk.Style()
		enabledLabelFrame_style.configure('enabled.TLabelframe', foreground=SECONDARY_LABEL_COLOR)
		disabledLabelFrame_style = ttk.Style()
		disabledLabelFrame_style.configure('disabled.TLabelframe', foreground=TERTIARY_LABEL_COLOR)
		self.textframe = ttk.LabelFrame(self,text=yaml_config['gui']['feature_descr']['label'],style='enabled.TLabelframe')
		self.text1frame = ttk.LabelFrame(self,text=yaml_config['gui']['requirement_loc']['label'],style='enabled.TLabelframe')
		self.text3frame = ttk.LabelFrame(self,text=yaml_config['gui']['verif_goals']['label'],style='enabled.TLabelframe')
		self.text4frame = ttk.LabelFrame(self,text=yaml_config['gui']['comments']['label'],style='enabled.TLabelframe')
		self.text5frame = ttk.LabelFrame(self,text=yaml_config['gui']['coverage_loc']['label'],style='enabled.TLabelframe')
		...
		self.settings_frame = ttk.LabelFrame(self.selector_canvas, text='Verification Approach', style='enabled.TLabelframe')
		self.pfc_frame = ttk.LabelFrame(self.settings_frame, text=yaml_config['gui']['pfc']['label'], style='success.TLabelframe')
		self.testtype_frame = ttk.LabelFrame(self.settings_frame, text=yaml_config['gui']['test_type']['label'], style='enabled.TLabelframe')
		self.covmethod_frame = ttk.LabelFrame(self.settings_frame, text=yaml_config['gui']['cov_method']['label'], style='enabled.TLabelframe')
		self.listofcores_frame = ttk.LabelFrame(self.selector_canvas, text='Applicable Cores', style='enabled.TLabelframe')
		self.cores_all = tk.IntVar(self.listofcores_frame)
		self.cores_all_btn = tk.Checkbutton(self.listofcores_frame, text=yaml_config['cores']['default']['label'],
			variable=self.cores_all, offvalue=0, onvalue=1, command=self.all_cores_btn_selected, state='active')
		self.core_canvas = tk.Canvas(self.listofcores_frame)
		self.core_button = []
		self.num_cores = len(yaml_config['cores']['values'])
		for i in range(self.num_cores):
			entry = yaml_config['cores']['values'][i]
			intvar = tk.IntVar(self.core_canvas)
			btn = tk.Checkbutton(self.core_canvas, text=entry['label'], variable=intvar, offvalue=0, onvalue=1, command=self.core_btn_selected)
		...
		self.text = MyText(self.textframe,cue_text=yaml_config['gui']['feature_descr']['cue_text'],state='disabled',height=6,bg=BG_COLOR,undo=True,wrap='word')
		self.text1 = MyText(self.text1frame,cue_text=yaml_config['gui']['requirement_loc']['cue_text'],state='disabled',height=4,bg=BG_COLOR,undo=True,wrap='word')
		self.text3 = MyText(self.text3frame,cue_text=yaml_config['gui']['verif_goals']['cue_text'],state='disabled',height=8,bg=BG_COLOR,undo=True,wrap='word')
		self.text4 = MyText(self.text4frame,cue_text=yaml_config['gui']['comments']['cue_text'],state='disabled',height=6,bg=BG_COLOR,undo=True,wrap='word')
		self.text5 = MyText(self.text5frame,cue_text=yaml_config['gui']['coverage_loc']['cue_text'],state='disabled',height=1,bg=BG_COLOR,undo=True,wrap='word')
		...
		self.keyevent_notify_list = [self.text, self.text1, self.text3, self.text4, self.text5]
		...
		self.bcancel = ttk.Button(self.bframe, text="Cancel", command=self.bcancel_action)
		...
		self.pfc_entries = sorted(yaml_config['gui']['pfc']['values'], key=lambda e: e['value'])
		self.num_pfcs = len(self.pfc_entries)

		# Create the PFC ComboBox
		self.pfc_string = tk.StringVar(self)
		self.pfc_cbox = ttk.Combobox(self.pfc_frame, textvariable=self.pfc_string, width=22)
		self.pfc_names = []
		for i in range(self.num_pfcs):
			self.pfc_names.append(self.pfc_entries[i]['label'])
		pfc_default = yaml_config['gui']['pfc']['default']
		...
		# Set default value
		self.pfc_cbox.set(yaml_config['gui']['pfc']['default']['label'])
		self.pfc = pfc_default['value']

		# Likewise for test type.
		self.testtype_entries = sorted(yaml_config['gui']['test_type']['values'], key=lambda e: e['value'])
		self.num_testtypes = len(self.testtype_entries)

		# Create test type Combobox.
		self.testtype_cbox = ttk.Combobox(self.testtype_frame, textvariable=self.testtype_string, width=22)
		self.testtype_names = []
		for i in range(self.num_testtypes):
			self.testtype_names.append(self.testtype_entries[i]['label'])
		testtype_default = yaml_config['gui']['test_type']['default']
		self.testtype_cbox['values'] = tuple([testtype_default['label']] + self.testtype_names)
		...
		self.covmethod_entries = sorted(yaml_config['gui']['cov_method']['values'], key=lambda e: e['value'])
		self.num_covmethods = len(self.covmethod_entries)

		# Create coverage method Combobox.
		self.covmethod_cbox = ttk.Combobox(self.covmethod_frame, textvariable=self.covmethod_string, width=22)
		self.covmethod_names = []
		for i in range(self.num_covmethods):
			self.covmethod_names.append(self.covmethod_entries[i]['label'])
		covmethod_default = yaml_config['gui']['cov_method']['default']
		self.covmethod_cbox['values'] = tuple([covmethod_default['label']] + self.covmethod_names)
		...
		self.pfc_func = pfc_func
		self.testtype_func = testtype_func
		self.covmethod_func = covmethod_func
	...
	def pfc_cbox_selected(self, event):
		...
		if not current_choice.startswith(yaml_config['gui']['pfc']['default']['label']):
			pfcs = [e['value'] for e in self.pfc_entries if e['label'] == current_choice]
			if pfcs:
				current_item = self.parent.item_widget.get_selection()
				if current_item and not self.parent.get_current_item().is_locked():
	...
	def testtype_cbox_selected(self, event):
		...
		if not current.startswith(yaml_config['gui']['test_type']['default']['label']):
			testtypes = [e['value'] for e in self.testtype_entries if e['label'] == current]
			if testtypes:
				current_item = self.parent.item_widget.get_selection()
				if current_item and not self.parent.get_current_item().is_locked():
	...
	def covmethod_cbox_selected(self, event):
		...
		if not current.startswith(yaml_config['gui']['cov_method']['default']['label']):
			covmethods = [e['value'] for e in self.covmethod_entries if e['label'] == current]
			if covmethods:
				current_item = self.parent.item_widget.get_selection()
				if current_item and not self.parent.get_current_item().is_locked():
	...
	def pack(self,side,padx,pady):
		...
		self.settings_frame.pack(side='left', padx=padx, pady=pady, fill='both', expand=True)
		...
		self.listofcores_frame.pack(side='left', padx=padx, pady=pady, fill='both', expand=True)
		self.cores_all_btn.pack(side='top', fill='x')
		self.separator_h2.pack(side='top', padx=padx, pady=pady, fill='both')
		self.core_canvas.pack(side='top', padx=padx, pady=pady, fill='both', expand=True)
		...
		ttk.LabelFrame.pack(self, expand=True, fill='both', side=side, padx=padx, pady=pady)
	...
	def pfc_action(self):
		if ismethod(self.pfc_func):
			self.pfc_func()		#execute external function before customization

	def testtype_action(self):
		if ismethod(self.testtype_func):
			self.testtype_func()		#execute external function before customization

	def covmethod_action(self):
		if ismethod(self.covmethod_func):
			self.covmethod_func()		#execute external function before customization
	...
	def clean(self):
		...
		self.pfc = yaml_config['gui']['pfc']['default']['value']
		self.pfc_cbox.set(yaml_config['gui']['pfc']['default']['label'])
		self.test_type = yaml_config['gui']['test_type']['default']['value']
		self.testtype_cbox.set(yaml_config['gui']['test_type']['default']['label'])
		self.test_type = yaml_config['gui']['cov_method']['default']['value']
		self.covmethod_cbox.set(yaml_config['gui']['cov_method']['default']['label'])
...
class MyMain():
	...
	def im_up_des_keypress(self,*args):
		for widget in self.desc_widget.keyevent_notify_list:
			...
			if widget['state'] == 'normal' and cur_text.startswith(widget.cue_text) and self.top.focus_get() == widget:
		...
		if current_item != "" and current_item != None and args[0].keycode != 37 and args[0].keycode != 109 and not self.get_current_item().is_locked() and not self.need_lock_ip():
	...
	def im_up_desc_keyup(self, *args):
		for widget in self.desc_widget.keyevent_notify_list:
			...
			if widget['state'] == 'normal' and cur_text == '\n' and self.top.focus_get() == widget:
	...
	def im_up_des_ctrl_lock(self,*args):
		current_item=self.item_widget.get_selection()
		if current_item != "" and current_item != None and not self.hold_state and not self.need_lock_ip():			# Note it can be a int
	...
	def create_ip(self):
		global yaml_config
		field_name = yaml_config['gui']['ip']['label']
		ip_name = tkinter.simpledialog.askstring('New '+field_name, prompt='Enter %s to create' % field_name.lower(),parent=self.top)
		ip_index=""
		#if STANDALONE_VERIF:
		...
		if ip_name in list(self.ip_list.keys()) or ip_name == "":
			tkinter.messagebox.showwarning("Warning","%s already existing" % field_name)
	...
	def update_ip_widget(self):
		...
		key_sorted_by_ip_num=sorted(list(self.ip_list.values()), key=lambda ip: ip.wid_order if isinstance(ip.wid_order, int) else int(ip.wid_order, base=10))
		list_to_print = []
		for ip_elt in key_sorted_by_ip_num:
			list_to_print.append(ip_elt.name)
		self.ip_widget.update_list(list_to_print,color_dict=LOCKED_IP_DICT,index=index_to_set)
		self.update_property_widget()
		if list(self.ip_list.keys()):
			self.ip_widget.bdel['state'] = self.ip_widget.bdel_default		    # IP deletion button enabled if existing IP
		else:
			self.ip_widget.bdel['state'] = 'disabled'		# IP deletion button disabled if no IP  
	...
	def create_prop(self):
		global CUSTOM_NUM, yaml_config
		""""""
		current_ip_name=self.ip_widget.get_selection()
		field_name = yaml_config['gui']['property']['label']
		prop_name = tkinter.simpledialog.askstring('%s name' % field_name,prompt='Enter %s to create' % field_name.lower(),parent=self.top)
		#Name existence check is done in IP class
		if prop_name != "" and prop_name != None:
			done=self.ip_list[self.ip_widget.get_selection()].add_property(prop_name,tag="",custom_num=CUSTOM_NUM)# get current IP Object, and run create_property
			if not done:
				tkinter.messagebox.showwarning("Warning","%s already existing" % field_name)
			self.update_property_widget()
		else:
	...
	def update_property_widget(self):
		...
		if current_ip_name:
			key_sorted_by_prop_num=sorted(list(self.ip_list[current_ip_name].prop_list.values()), key=lambda prop: prop.wid_order)
			list_to_print = []
			...
			if list(self.ip_list[current_ip_name].prop_list.keys()) == []:
				self.prop_widget.bdel['state'] = 'disabled'		# if no Property exists, disable del button
			else:
				self.prop_widget.bdel['state'] = 'normal'
		else:												
			self.prop_widget.update_list([])						# if no IP exists, clear Property
			self.prop_widget.badd['state'] = 'disabled'			# if no IP exists, disable add/del button
	...
	def get_current_prop(self):
		return(self.ip_list[self.ip_widget.get_selection()].prop_list[self.prop_widget.get_selection()])

	#####################################
	### Item Section
	def create_item(self):
		""""""
		#item numbering is directly handled in class
		self.get_current_prop().add_item(tag=self.get_current_prop().tag)	# get current IP/Prop object
		print('### ==> DONE with add_item')
		self.update_item_widget()
		print('### ==> DONE with update_item_widget')
	...
	def update_item_widget(self):
		...
		if current_prop_name:
			...
			if self.item_widget.get_filter_vip() == 'ALL':
				...
				first_filtered_list=[x for x in list(self.get_current_prop().item_list.values()) if x.is_vip()]
			else: ##"ASSERTION"
				first_filtered_list=[x for x in list(self.get_current_prop().item_list.values()) if x.is_assert()]
			## Apply Second filter
			# Not done items only
			if filter_status == 'VALIDATED':
				...
				for i in [x for x in first_filtered_list if x.get_verif_status()[1] == ' Not Done']:
					fill_list.append(i.name)
			# Done items only
			...
			# get index to set if in line with history content
			if (self.ip_widget.get_index(),self.prop_widget.get_index()) in self.gui_item_history:
				index_to_set = self.gui_item_history[(self.ip_widget.get_index(),self.prop_widget.get_index())]
			else:
				index_to_set=0
		...
		if current_ip_name:
			if list(self.ip_list[current_ip_name].prop_list.keys()) == []:
				self.item_widget.badd['state'] = 'disabled'			# if no ip existing disable property/item creation
			else:
	...
	def duplicate_item(self,insert='no'):
		...
		if current_item_name and not self.need_lock_ip():
			...
			if insert == 'yes':
				confirm=tkinter.messagebox.askquestion("Reorder item numbering","Please take care while reordering items, Do you want to continue?")
				if confirm == 'yes':
					self.get_current_prop().add_item(tag=self.get_current_prop().tag,description=current_item.description,purpose=current_item.purpose)	# get current IP/Prop object
	...
	def get_current_item(self):
		try:
			return(self.ip_list[self.ip_widget.get_selection()].prop_list[self.prop_widget.get_selection()].item_list[self.item_widget.get_selection()])
		except:
			return None
	...
	def update_desc_widget(self):
		...
		current_prop_name=self.prop_widget.get_selection()
		current_item=self.item_widget.get_selection()
		if current_item != "" and current_item != None:						# Note it could be a int
			self.desc_widget.enable()
			# ZC: FIXME: Rework to hide internal structure of desc_widget.
			self.desc_widget.update_text(self.desc_widget.text, self.get_current_item().description)
			self.desc_widget.update_text(self.desc_widget.text1, self.get_current_item().purpose)
			self.desc_widget.update_text(self.desc_widget.text3, self.get_current_item().verif_goals)
			self.desc_widget.update_text(self.desc_widget.text4, self.get_current_item().comments)
			self.desc_widget.update_text(self.desc_widget.text5, self.get_current_item().coverage_loc)
			pfc = self.get_current_item().pfc
			self.desc_widget.update_pfc(pfc)
			test_type = self.get_current_item().test_type
			self.desc_widget.update_testtype(test_type)
			cov_method = self.get_current_item().cov_method
			self.desc_widget.update_covmethod(cov_method)
			try:
				cores = self.get_current_item().cores
			except:
				self.get_current_item().cores = -1
				cores = -1
			self.desc_widget.update_cores(cores)

			if self.get_current_item().is_locked():
	...
	def desc_save(self):
		# ZC: FIXME: Rework to hide internal structure of desc_widget.
		self.get_current_item().description=self.desc_widget.text.get(0.0,tk.END).rstrip()
		self.get_current_item().purpose=self.desc_widget.text1.get(0.0,tk.END).rstrip()
		self.get_current_item().verif_goals=self.desc_widget.text3.get(0.0,tk.END).rstrip()
		self.get_current_item().comments=self.desc_widget.text4.get(0.0,tk.END).rstrip()
		self.get_current_item().coverage_loc=self.desc_widget.text5.get(0.0,tk.END).rstrip()
		self.unfreeze_all()
	...
	def update_pfc(self):
		...
		# ZC FIXME ??? Do we really need this.
	...
	def get_db_gitrev(self):
		...
		db_filenames = []
		if self.split_save:
			dir_to_load=os.path.dirname(SAVED_DB_LOCATION)
			db_filenames = glob.glob(dir_to_load+"/VP_IP*pck")
		else:
			db_filenames = [SAVED_DB_LOCATION]
		working_dir=os.getcwd()
		ip_dir = os.path.dirname(SAVED_DB_LOCATION)
		os.chdir(ip_dir)
		sha1_dict = OrderedDict()
		for db_fname in db_filenames:
			try:
				if subprocess.check_output("git ls-files " + db_fname, shell=True).decode('utf-8'):
					db_git_rev = subprocess.check_output("git log " + db_fname, shell=True).decode('utf-8').split(" ")[1]
					if db_git_rev:
						sha1_dict[db_fname] = db_git_rev
				else:
					print("WARNING: DB file '%s' is not under Git version control!" % db_fname)
			except Exception as e:
				print("WARNING: Loaded DB is not under consistent git versioning (exception '%s')" % str(e))
		os.chdir(working_dir)
		return sha1_dict

	def save_db(self,save_as='no'):
		global SCRIPT_LOCATION, SAVED_DB_LOCATION, yaml_config
		pickle_ip_list = []
		confirm = ''
		if self.ip_list:
			if save_as == "no":
				confirm=tkinter.messagebox.askquestion("Saving current Database","Do you really want to Save?")
			else:
				db_to_open = asksaveasfilename(initialdir=VP_PLATFORM_TOP_DIR)
				if db_to_open:
					open(db_to_open,'wb').close()	# create file to save
					if os.path.exists(db_to_open):
						SAVED_DB_LOCATION = db_to_open
						confirm = "yes"
			if confirm == "yes":
				...
				if self.db_git_rev == current_db_revision or save_as != "no":
					...
					self.prep_db_export()
					pickle_ip_list = sorted(list(self.ip_list.items()),key= lambda key: key[0])
					if self.split_save:
						save_dir=os.path.dirname(SAVED_DB_LOCATION)
						saved_ip_str=""
						for ip_elt in pickle_ip_list:
							...
							if self.is_locked_by_user(current_ip_name=ip_elt[1].name):
								...
								with open(save_dir+"/VP_IP"+str(ip_elt[1].ip_num).zfill(3)+".pck", 'wb') as output:
									pickle.dump(ip_elt, output, 0)
						if saved_ip_str:
							tkinter.messagebox.showinfo("Info","Following %s(s) saved: %s" % (yaml_config['gui']['ip']['label'].lower(), saved_ip_str))
						else:
							tkinter.messagebox.showwarning("Warning","No %s(s) were locked and then saved" % yaml_config['gui']['ip']['label'].lower())
					else:
						with open(SAVED_DB_LOCATION, 'wb') as output:
					...
					tkinter.messagebox.showwarning("Warning","DB git revision has been updated behind our back! DB can not be saved! Initial/current hash set: see CONSOLE")
					print("WARNING: DB git revision has been updated behind our back, DB can not be saved!\n" + \
					 "\n\tInitial hash set = " + str(self.db_git_rev) + "\n" + \
						 "\n\tNew hash set = " + str(current_db_revision))
		else:
	...
	def load_db(self,):
		global SCRIPT_LOCATION, SAVED_DB_LOCATION, VP_PLATFORM_TOP_DIR
		need_to_load=""
		if self.ip_list:
			need_to_load=tkinter.messagebox.askquestion("Loading Database", "Close current DB and Load?")
		else:
			need_to_load="yes"
		if need_to_load == "yes":
			db_to_open = askopenfilename(initialdir=VP_PLATFORM_TOP_DIR)
	...
	def load_db_quiet(self):
		...
		if self.split_save:
			dir_to_load=os.path.dirname(SAVED_DB_LOCATION)
			self.top.title('%s: %s' % (yaml_config['gui']['title'] + ' (project: ' + (PROJECT_NAME if PROJECT_NAME else '<generic>') + ')', os.path.join(dir_to_load, 'VP_IP*.pck')))
			for filename in glob.glob(dir_to_load+"/VP_IP*pck"):
				#print("---INFO: Loading "+filename)
				with open(filename, 'rb') as input:
					pickle_ip_list.append(pickle.load(input))
			if pickle_ip_list:
				# Explicit ip_nums can be strings iso. ints.
				ip_amount=max([(lambda e: e if isinstance(e, int) else int(e, base=10))(elt[1].ip_num) for elt in pickle_ip_list])
		else:
			#if a specific db to open is passed through argument update global var, for future saving
			self.top.title('%s: %s' % (yaml_config['gui']['title'], SAVED_DB_LOCATION))
			with open(SAVED_DB_LOCATION, 'rb') as input:
				ip_amount=pickle.load(input)
				for dummy in range(ip_amount):
					pickle_ip_list.append(pickle.load(input))
		# change list to dict
		for ip_key,ip_elt in pickle_ip_list:
			...
			self.ip_list[ip_key].__class__._ip_count=ip_amount
		self.prep_db_import()
		self.update_all_item_target_list()
	...
	def close_db(self):
		confirm = "no"
		if self.ip_list:
			confirm=tkinter.messagebox.askquestion("Closing current Database","Do you really want to Close Current Db?")
			if confirm == "yes":
				self.close_db_quiet()
		return confirm
	...
	def rename_ip(self):
		...
		new_name = tkinter.simpledialog.askstring('Renaming IP',prompt='Specify a new name for IP',parent=self.top,initialvalue=initial_name)
		if new_name:
			self.update_ip_name(initial_name,new_name)
			self.update_ip_widget()
	def lock_ip(self):
		global LOCKED_IP_DICT, LOCKED_IP_LOCATION
		current_ip_name=self.ip_widget.get_selection()
		...
		if current_ip_name in LOCKED_IP_DICT:
			if LOCKED_IP_DICT[current_ip_name] == pwd.getpwuid(os.getuid() ).pw_name:
				...
				tkinter.messagebox.showwarning("Warning","Ip is locked by "+LOCKED_IP_DICT[current_ip_name])
		else:
			LOCKED_IP_DICT[current_ip_name]=pwd.getpwuid(os.getuid() ).pw_name
		try:
			with open(LOCKED_IP_LOCATION, 'wb') as output:
				pickle.dump(LOCKED_IP_DICT, output, 0)
			self.update_ip_widget()
		except Exception as e:
			print("WARNING: Locked Ip file has not been loaded (exception '%s')" % str(e))
	...
	def load_lock_ip(self):
		...
		try:
			with open(LOCKED_IP_LOCATION, 'rb') as input:
	...
	##### Main GUI Creation	
	def create_gui(self):
		global yaml_config, PROJECT_NAME
		self.personalization()

		# TOP Widget
		self.top = themed_tk.ThemedTk(toplevel=True, className='cv-vptool', theme=yaml_config['gui']['theme'])
		print('### Available themes: %s' % self.top.get_themes())
		self.top.title(yaml_config['gui']['title'] + ': ' + (PROJECT_NAME if PROJECT_NAME else '<generic project>'))
		self.top.minsize(1200,660)

		self.icon = ImageTk.PhotoImage(file=os.path.join(os.path.dirname(__file__), "vptool-icon.ico"))
		self.top.iconphoto(True, self.icon)
		# IP Widget
		self.ip_widget = MyListWidget(master=self.top, MasterClass=self,reflist=list(self.ip_list.keys()),text=yaml_config['gui']['ip']['label'],side="left",expand="no",baddfunc=self.create_ip,bdelfunc=self.delete_ip,bdel_default='disabled')
		self.ip_widget.badd['state'] = 'normal'		# IP creation button enabled
		# Property Widget
		self.prop_widget = MyListWidget(master=self.top, MasterClass=self,text=yaml_config['gui']['property']['label'],side="left",expand="no",baddfunc=self.create_prop,bdelfunc=self.delete_prop,width=35)
		# Item Widget
		self.item_widget = MyListWidget(master=self.top, MasterClass=self,text=yaml_config['gui']['item']['label'],side="left",expand="no",baddfunc=self.create_item,bdelfunc=self.delete_item,width=6,spinfunc=self.update_item_widget,spin_value=None,spin_vip_value=None)
		self.verif_menu=MyMenuWidget(master=self.top, MasterClass=self, savfunc=self.save_db, \
			loadfunc=self.load_db, closefunc=self.close_db, renamefunc=self.rename_ip, \
				lockfunc=self.lock_ip, duplicate_itemfunc=self.duplicate_item, \
		...
		self.desc_widget=MyTextWidget(master=self.top, parent=self, text=yaml_config['gui']['test_descr']['label'],side="top",bsavefunc=self.desc_save,bcancelfunc=self.desc_cancel,pfc_func=self.update_pfc,testtype_func=self.update_testtype,covmethod_func=self.update_covmethod)
		# Layout
...
def __generate_option_parser():
	...
	parser.add_option('-d','--db', dest='dataBase',
				help='Specify a DataBase to load', default='empty', metavar='DATABASE')
	return parser
...
try:
	with open(os.path.join(os.path.dirname(__file__), YAML_CONFIG_FILE), 'r') as f:
		...
		#print("YAML config = \n" + dump(yaml_config, Dumper=Dumper))
except Exception as e:
	print("Cannot load YAML configuration file '%s' (exception: %s)" % (YAML_CONFIG_FILE, str(e)))
	yaml_config = None

top_gui=MyMain(split_save=SPLIT_SAVE)
