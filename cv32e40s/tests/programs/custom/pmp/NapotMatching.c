...
extern uint32_t calc_size(unsigned int cfg);
extern int calc_top(int cfg);
extern int calc_base(int cfg);
...
void napot_matching()
{
  int base_addr, top_addr, region_size, temp;
  int count = 0;
  // PMPXCFG pmpxcfg;
  // PMPCFGX pmpcfgx;
  static CSRS csrs;
  // 2 set bits
  unsigned int mode = ((1 << 4) | (1 << 3)), exe = 1 << 2, write = 1 << 1, read = 1;
  ...
  base_addr = calc_base(ABOVEDEBUG);
  top_addr = calc_top(ABOVEDEBUG);

  printf("\t\n base addr 0x%x\n", base_addr);

  // store word to memory base_addr
  for (int i = base_addr; i <= top_addr; i += 4)
  {
    store2addr(i - base_addr + 100, i);
    // printf("\t\n current address %x", i);
